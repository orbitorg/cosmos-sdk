...
if [ $# -lt 3 ]; then
    echo "Usage: update-release.sh <branch> <simapp_version> <target-directory> [<go_mod_name1> <go_mod_name2> ...]"
    exit 1
fi
...
# keep only simapp v2
git filter-branch --subdirectory-filter simapp/$simapp_version
rm -rf .git
